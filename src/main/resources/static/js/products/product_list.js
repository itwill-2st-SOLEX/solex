...
const bgrid = new tui.Grid({
    ...
    data: [],
    columns: [
		{ header: 'BOM번호', name: '', align : 'center' },
		{ header: '원자재', name: '',align : 'center', sortable: 'true' },
		{ header: '제품', name: '',align : 'center', sortable: 'true' },
		{ header: '단위', name: '',align : 'center', sortable: 'true' },
		{ header: '소모량', name: '', sortable: 'true',align : 'center' },
		{ header: '설명', name: '', sortable: 'true',align : 'center' },
		{ header: '등록일', name: '', sortable: 'true',align : 'center' }
	]
});
...
async function bomList(page) {
    try {
		...
		// 무한스크롤 페이지, 검색어 url로 전달
		let url = `/SOLEX/boms/api/productList?page=${page}&size=${pageSize}`;

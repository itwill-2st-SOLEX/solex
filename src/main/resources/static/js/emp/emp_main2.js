document.addEventListener('DOMContentLoaded', function(){
	...
	const registerBtn = document.getElementById('registerBtn');   // 등록 버튼
...
//			form.setAttribute('action', empData ? '/emp/update' : '/emp/registration');
			...
			form.addEventListener('submit', async (e) => {
			  ...
			  // 방법 1: input 필드의 값을 먼저 YYYY-MM-DD로 변경 후 FormData 생성
				...
				document.getElementById('emp_phone').value = phoneCombined;
				...
				// ✅ 이메일 조합
				 ...
				 const emailCombined = `${email1}@${email2}`;

				 document.getElementById('emp_email').value = emailCombined;
			 ...
			 const formdateData = new FormData(form); // 이제 FormData에 YYYY-MM-DD가 들어감

			  const url = empData ? '/SOLEX/emp/modify' : '/SOLEX/emp/registration'; 
			  const method = empData ? 'PUT' : 'POST'; // 수정은 PUT, 등록은 POST (RESTful API 권장)
			  
			  const formData = new FormData(this); // 현재 폼의 모든 입력 필드 값을 FormData 객체로 가져옵니다.
			  
			  // 2. 서버로 보낼 JSON 페이로드 구성 (핵심 데이터)
			  const payload = {
			  ...
			  // `empData`가 있을 경우, 수정할 직원의 고유 식별자(예: emp_num)를 `data` 객체에 추가해야 합니다.
			  if(empData && empData.EMP_NUM){
				data.emp_num = empData.EMP_NUM;
			  }
			  
			  // 사원, 부서, 직급, 팀 코드만 보낼 경우, data 객체에 해당 값만 있는지 확인하세요.
		      // 만약 다른 필드도 폼에 있다면 `formData`는 모든 필드를 포함할 것이므로,
		      // 필요한 데이터만 필터링하거나 백엔드에서 필요한 필드만 사용하도록 처리해야 합니다.
			  
			  console.log('서버로 보낼 데이터 (Payload):', payload);
			  try {
			        const response = await fetch(url, {
			            method: method, // PUT 또는 POST
			            headers: {
			                'Content-Type': 'application/json' // 서버에 JSON 형식으로 데이터를 보낸다고 알립니다.
			            },
			            body: JSON.stringify(payload) // JavaScript 객체를 JSON 문자열로 변환하여 보냅니다.
			        });

			        if (!response.ok) { // 응답 상태 코드가 200번대가 아니면 오류 처리
			            const errorData = await response.json(); // 서버에서 보낸 에러 메시지가 있다면 파싱
			        ...
			        // 성공 처리 (예: 모달 닫기, 메시지 표시, 직원 목록 새로고침)
			        alert(result.message || (empData ? '직원 정보가 성공적으로 수정되었습니다!' : '직원이 성공적으로 등록되었습니다!'));
			        ...
			        // TODO: 직원 목록을 새로고침하는 함수 호출 (예: `loadEmployeeList();`)
			        // 이 부분은 실제 페이지 구조와 연동하여 구현해야 합니다.

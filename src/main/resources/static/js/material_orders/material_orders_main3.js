$(function() {
	...
	async function fetchAndPopulateWarehouse(matId) {
		...
		try{
			const response = await fetchJson(`/SOLEX/material_orders/getWarehouse?matId=${matId}`, []); // 자재목록을 가져올 api 엔드포인트에 요청

			response.forEach(war => {
				$('<option>', {
		        	value: war.WHS_ID,                // 실제 값 
		        	text : war.WHS_NM      // 사용자에게 보이는 글자
		      	}).appendTo($sel);
		    });
	  ...
	  async function fetchAndPopulateArea(whsId, matId) {

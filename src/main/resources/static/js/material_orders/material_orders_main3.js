document.addEventListener('DOMContentLoaded', function(){
	...
	async function fetchAndPopulateMaterial(selectElement) {
		try {
			...
			console.error('자재목록 로딩 실패:', error);
		}
	}			
	...
	document.addEventListener('click', event => {
		const btn = event.target.closest('button');      // 버튼이 아닌 경우 무시
		if (!btn) return;
		
		if (btn.name === 'approval') {
		  const rowKey = btn.dataset.rowKey;             // 렌더러에서 달아 둔 값
		  openModal('approval', rowKey);                 // ← 모드와 rowKey 전달
